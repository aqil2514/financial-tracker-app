...
import { useRetailkuArAp } from "../hooks/use-retailku-ar-ap";
...
export function ArApTab() {
  ...
  if (!data || data.parties.length === 0) {
    return (
      <p className="text-muted-foreground text-sm">
        Tidak ada piutang/utang outstanding saat ini.
      </p>
    );
  }
      ...
      <div className="flex flex-wrap gap-4 text-sm">
        <div>
          ...
          <span className="font-medium">
            {formatCurrency(data.totalReceivable, "IDR")}
          </span>
        </div>
        <div>
            ...
            <TableRow key={party.id}>
              ...
              <TableCell>
                <Badge variant="secondary">
                  {party.type === "CUSTOMER" ? "Pelanggan" : "Pemasok"}
                </Badge>
              </TableCell>
              <TableCell className="text-right">
                {formatCurrency(party.outstandingReceivable, "IDR")}
              </TableCell>
              <TableCell className="text-right">
                {formatCurrency(party.outstandingPayable, "IDR")}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
