...
/**
 * Daftar ringkasan SEMUA key yang dimuat, masing-masing dengan nomor
 * urut yang sama dipakai label tab (lihat array-field-tabs.tsx) — BEDA
 * dari sekadar menampilkan ulang isi form key yang lagi aktif (itu
 * cuma duplikasi form itu sendiri, tidak berguna). Fungsinya: table-of-
 * contents yang otomatis ikut berubah (React re-render, bukan
 * `watch()`) begitu user mengisi form tab mana pun, supaya user tetap
 * punya gambaran menyeluruh — termasuk key yang BELUM aktif dilihat —
 * tanpa perlu klik satu-satu. Icon info di tiap baris menampung SEMUA
 * nilai form (akun/judul/kategori/deskripsi) dalam satu tooltip, supaya
 * baris daftar sendiri bisa tetap ringkas satu baris.
 */
              ...
              <li key={row.key} className="flex items-stretch">
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate font-medium">{row.accountName}</span>
