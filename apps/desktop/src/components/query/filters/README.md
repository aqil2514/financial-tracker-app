...
**`builders/sql.ts`** — `buildWhereClause(filters, allowedColumns, extraConditions?, startIndex?)`:
...
- **`startIndex`** (opsional, default `1`) untuk kasus hasil builder ini digabung dengan params dari builder/klausa lain dalam query yang sama — mis. `use-accounts.ts` memanggil `buildWhereClause` dua kali dalam satu query (sekali untuk `WHERE` kolom asli, sekali lagi untuk `HAVING` field computed seperti `balance`) dan perlu param kedua melanjutkan nomor placeholder dari yang pertama, bukan mulai dari `$1` lagi. Isi dengan `params.length + 1` dari builder sebelumnya.
